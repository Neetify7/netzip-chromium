chrome.devtools.panels.create("NetZIP", "", "devtools.html", panel => {
...
document.getElementById("downloadCurrent").onclick = async () => {
  ...
  const origin = await new Promise(resolve => {
    chrome.devtools.inspectedWindow.eval("location.origin", result => resolve(result));
  });
...
document.getElementById("crawlSite").onclick = async () => {
  ...
  const currentUrl = await new Promise(resolve => {
    chrome.devtools.inspectedWindow.eval("location.href", result => resolve(result));
  });
  ...
  while (toVisit.length && crawling) {
    ...
    if (currentLocation !== url) {
      await chrome.tabs.update(inspectedTabId, { url });
      await sleep(3000);
    }
